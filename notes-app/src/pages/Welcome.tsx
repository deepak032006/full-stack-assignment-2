...
function Welcome() {
  ...
  const fetchNotes = async () => {
    try {
      const res = await axios.get("https://full-stack-assignment-2-0n7g.onrender.com/api/notes", {
  ...
  const fetchUser = async () => {
    try {
      const res = await axios.get("https://full-stack-assignment-2-0n7g.onrender.com/api/auth/me", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const createNote = async () => {
    ...
    try {
      const res = await axios.post(
        "https://full-stack-assignment-2-0n7g.onrender.com/api/notes",
        { content: newNote },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const deleteNote = async (id: string) => {
    try {
      await axios.delete(`https://full-stack-assignment-2-0n7g.onrender.com/api/notes/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
